...
public:
  ...
  static constexpr char LIST_SIZE_TRACKING_BUG_MESSAGE[] = "Error: The list size is not correctly being updated";
  ...
  void clear() {
    ...
    if (0 != size_) throw std::runtime_error(LIST_SIZE_TRACKING_BUG_MESSAGE);
  }
...
template <typename T>
constexpr char LinkedList<T>::LIST_SIZE_TRACKING_BUG_MESSAGE[];
...
bool LinkedList<T>::isSorted() const {
  ...
  if (!head_) throw std::runtime_error(LIST_SIZE_TRACKING_BUG_MESSAGE);
...
bool LinkedList<T>::equals(const LinkedList<T>& other) const {
  ...
  while (thisCur) {
    if (!otherCur) {
      throw std::runtime_error("otherCur missing a node or wrong item count");
    }
...
bool LinkedList<T>::assertCorrectSize() const {
  ...
  if (itemCount != size_) throw std::runtime_error(LIST_SIZE_TRACKING_BUG_MESSAGE);
  else return true;
}
...
bool LinkedList<T>::assertPrevLinks() const {
  ...
  else throw std::runtime_error("Error in assertPrevLinks: tail_ pointer is wrong, or some prev link is wrong, or the list size is wrong");
}
